...
import { Section, InfoRow, ExternalLink } from "./public-event-primitives";
import { formatDate, formatTime, formatDateTime, toTitleCase } from "./public-event-helpers";

// ─── Location ─────────────────────────────────────────────────────────────────

function LocationContent({ loc, isPhysical, isVirtual, isHybrid }: {
...
// ─── Ticket plans (public — no seat management UI) ────────────────────────────

function PublicTicketPlans({ plans }: { plans: any[] }): JSX.Element {
  return (
    <Section icon={TicketIcon} title="Ticket Plans">
      <div className="space-y-3">
        {plans.map((plan: any, i: number) => {
          ...
          const earlyLabel    = plan.earlyBirdDeadline ? formatDateTime(String(plan.earlyBirdDeadline)) : null;
            ...
            <div key={i} className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-800">
                ...
                <div className="text-right">
                  <p className={`text-sm font-bold ${plan.price === 0 ? "text-green-600 dark:text-green-400" : "text-gray-900 dark:text-white"}`}>
                    {planPrice}
                  </p>
                  {soldOut    && <p className="text-xs text-red-500 mt-0.5">Sold out</p>}
                  {almostGone && !soldOut && <p className="text-xs text-amber-500 mt-0.5">{seatsLeft} left!</p>}
                </div>
              </div>
              ...
              {plan.benefits?.length > 0 && (
                <div className="px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700 flex flex-wrap gap-1.5">
                  {(plan.benefits as string[]).map((b: string, bi: number) => (
              ...
              {earlyLabel && (
                <div className="px-4 py-2 bg-amber-50 dark:bg-amber-900/10 border-t border-amber-100 dark:border-amber-800">
                  <p className="text-xs text-amber-700 dark:text-amber-400">Early bird deadline: {earlyLabel}</p>
                </div>
              )}
            </div>
          );
        ...
        {speakers.map((s: any, i: number) => (
          <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
            ...
            <div className="min-w-0 flex-1">
              ...
              {s.company && <p className="text-xs text-gray-500 dark:text-gray-400">{s.company}</p>}
              {s.bio     && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5 line-clamp-2">{s.bio}</p>}
            </div>
          </div>
        ))}
      </div>
...
interface PublicEventLeftColumnProps {
  event: any; loc: any;
  isPhysical: boolean; isVirtual: boolean; isHybrid: boolean;
  locationTitle: string; locationIcon: any;
  speakers: any[]; tags: string[];
  ageReq: any;
  regDeadline: string | undefined;
  timezone: string;
}

export function PublicEventLeftColumn({
  event, loc, isPhysical, isVirtual, isHybrid,
  locationTitle, locationIcon, speakers, tags,
  ageReq, regDeadline, timezone,
}: PublicEventLeftColumnProps): JSX.Element {
  return (
    <div className="lg:col-span-2 space-y-5">

      {/* About */}
      <Section icon={InformationCircleIcon} title="About This Event">
      ...
      <Section icon={CalendarIcon} title="Date & Time">
        ...
        {regDeadline && <InfoRow label="Register by" value={formatDateTime(regDeadline)} />}
      </Section>
      ...
      {/* Ticket plans */}
      <PublicTicketPlans plans={event.plans ?? []} />

      {/* Speakers */}
      <Speakers speakers={speakers} />

      {/* Age requirements — show publicly so attendees know before registering */}
      {ageReq?.required && (
        <Section icon={ShieldUserIcon} title="Age Requirements">
          {ageReq.minAge && <InfoRow label="Minimum Age" value={`${ageReq.minAge} years old`} />}
          {ageReq.maxAge && <InfoRow label="Maximum Age" value={`${ageReq.maxAge} years old`} />}
          {ageReq.requiresParentalConsent && (
            <InfoRow label="Parental Consent" value={ageReq.parentalConsentMessage ?? "Required for under-18 attendees"} />
          )}
        </Section>
      )}
    </div>
