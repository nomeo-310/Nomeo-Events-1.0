...
type NestedModalSize = "small" | "medium" | "large" | "extra-large" | "extremely-large";

const modalSizes: Record<NestedModalSize, { width: string; maxHeight: string; padding: string; titleSize: string; descriptionSize: string; }> = {
  small: {
    width: "max-w-md",
...
interface NestedModalHeaderProps {
  title: string;
  description?: string;
  icon?: any;
...
const NestedModalHeader = ({ title, description, icon: Icon, showCloseButton, onClose, size }: NestedModalHeaderProps) => {
...
interface NestedModalContentProps {
  children: ReactNode;
  size: NestedModalSize;
}

const NestedModalContent = ({ children, size }: NestedModalContentProps) => {
  const sizeConfig = modalSizes[size];
...
export const NestedModal = () => {
  const { state, closeNestedModal } = useNestedModalStore();
  ...
  }, [state.isOpen, state.closeOnEsc, closeNestedModal]);

  // Don't lock body scroll for nested modals - only main modal does that
  // This prevents double scroll locking

  if (!mounted) return null;

  const sizeConfig = modalSizes[state.size];

  return (
    <AnimatePresence>
      {state.isOpen && (
        <>
          {/* Overlay - Higher z-index to appear above main modal */}
          <motion.div
            ...
            onClick={state.closeOnOutsideClick ? closeNestedModal : undefined}
          />
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
              ...
              {/* Content */}
              <NestedModalContent size={state.size}>
                {state.children}
              </NestedModalContent>
            </motion.div>
          </div>
