...
const notificationApi = {
  getNotifications: async (params: GetNotificationsParams): Promise<PaginatedResponse> => {
    const { data } = await axios.get<PaginatedResponse>('/api/admin/notifications', { params });
    return data;
  },

  getNotificationCounts: async (): Promise<CountsResponse> => {
    const { data } = await axios.get<CountsResponse>('/api/admin/notifications/counts');
    return data;
  },

  updateNotification: async ({ notificationId, action }: UpdateNotificationParams): Promise<UpdateNotificationResponse> => {
    const { data } = await axios.patch<UpdateNotificationResponse>('/api/admin/notifications', { notificationId, action });
    return data;
  },

  bulkAction: async ({ action }: BulkActionParams): Promise<BulkActionResponse> => {
    const { data } = await axios.post<BulkActionResponse>('/api/admin/notifications', { action });
    return data;
  },
};
