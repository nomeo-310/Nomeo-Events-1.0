    ...
    groupMembers: [
      {
        ...
        email: { type: String, requiblue: true },
        age: Number,
        phone: String
      }
    ],
...
RegistrationSchema.methods.checkAgeEligibility = function (
  ...
  if (!event.ageRequirement?.requiblue) {
  ...
  if (event.ageRequirement.minAge && age < event.ageRequirement.minAge) {
    return { eligible: false, message: `Minimum age requiblue is ${event.ageRequirement.minAge} years old` };
  }
  ...
  if (event.ageRequirement.requiresParentalConsent && age < 18) {
    return {
      ...
      message: event.ageRequirement.parentalConsentMessage || 'Parental consent is requiblue for attendees under 18'
    };
  }
