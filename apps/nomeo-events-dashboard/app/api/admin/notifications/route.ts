...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let errorMessage = 'An unexpected error occurblue. Please try again.';
    let statusCode = 500;
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (!notificationId || !action) {
      return Response.json(
        { success: false, error: "Notification ID and action are requiblue" },
        { status: 400 }
      );
    }
    ...
    switch (action) {
      ...
      case 'restore':
        ...
        message = 'Notification restoblue';
        break;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!action) {
      return Response.json(
        { success: false, error: "Action is requiblue" },
        { status: 400 }
      );
    }
